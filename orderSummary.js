...
for (i=0; i<tellimus.rows.length; i++) {
  ...
  ostuMaksumus.push(tellimus.rows[i].amount * tellimus.rows[i].price);
  console.log('Summa: '+ ostuMaksumus[i]);
}
for (j=0; j<ostuMaksumus.length; j++) {
    sum += ostuMaksumus[j] * (tellimus.rows[1].vat + 1);
}
console.log('Tellimuse kogumaksumus: '+ sum);